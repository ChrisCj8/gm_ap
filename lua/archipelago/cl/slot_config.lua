...
function BuildSlotConfigMenu(panel)
    timer.Simple( 0, function() -- for some fucking reason the text inputs won't generate properly if i don't do this, there's probably a better fix but idk what it os
        ...
        local function GenerateTextInput(labelText)
            ...
            TextInput:SetTall(20)
                ...
                --TextInput:Add(TextInput.Label)

                TextInput.Input = vgui.Create("DTextEntry", TextInput)
                TextInput.Input:AlignLeft(labelwidth + 20)
                TextInput.Input:StretchToParent(nil,0,20)
                --TextInput:Add(TextInput.Input)
